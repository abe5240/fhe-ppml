...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(3, 9, sharex=True, figsize=(16,6))
    ...
    for i in range(2,11):
        ...
        if i == 2:
            axs[0][i-2].plot(xs, np.where(xs>0,xs,xs*0.1), '--', color='tab:red', label='True', zorder=1, linewidth=2)
            axs[0][i-2].plot(xs, lrelu_out, color='tab:blue', label='Poly.')

            axs[1][i-2].plot(xs, 1/(1+np.exp(-xs)), '--', color='tab:red', zorder=1, linewidth=2)
            axs[1][i-2].plot(xs, sigmoid_out, color='tab:blue')

            axs[2][i-2].plot(xs,  np.log(1+np.exp(xs)), '--', color='tab:red', zorder=1, linewidth=2)
            axs[2][i-2].plot(xs, softplus_out, color='tab:blue')         

        else:
            axs[0][i-2].plot(xs, np.where(xs>0,xs,xs*0.1), '--', color='tab:red', zorder=1, linewidth=2)
            axs[1][i-2].plot(xs, 1/(1+np.exp(-xs)), '--', color='tab:red', zorder=1, linewidth=2)
            axs[2][i-2].plot(xs, np.log(1+np.exp(xs)), '--', color='tab:red', zorder=1, linewidth=2)
    ...
    axs[0][0].legend()
    axs[2][4].set_xlabel("x")
    plt.tight_layout()
    plt.savefig('polys.eps')
